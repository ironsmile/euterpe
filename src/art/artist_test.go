...
func TestClientGetArtistImage(t *testing.T) {
	...
	c := NewClient(userAgent, 0, discogsToken)
	c.musicBrainzAPIHost = mbrainz.URL
	c.discogsAPIHost = discogs.URL
	c.discogsAuthToken = discogsToken
...
func TestClientNoDiscogsAuth(t *testing.T) {
	c := NewClient("euterpe/testing", 1*time.Millisecond, "")
	buff, err := c.GetArtistImage(context.Background(), "Iron Maiden")

	if !errors.Is(err, ErrNoDiscogsAuth) {
